...
class EventsViewModel(private val repository: EventsRepository) : ViewModel() {
    ...
    private val _nearestActiveEvent = MutableLiveData<EventsResponse>()
    val nearestActiveEvent: LiveData<EventsResponse> = _nearestActiveEvent
    ...
    fun fetchNearestActiveEvent() {
        viewModelScope.launch {
            try {
                val response = repository.getNearestActiveEvent()
                _nearestActiveEvent.value = response
            } catch (e: Exception) {
                Log.e("EventsViewModel", "Error fetching nearest active event", e)
                _errorMessage.value = Event(e.message ?: "Get Error.")
            }
        }
    }
